...
export default function MainDocs({schema}: DocsType){
  ...
  function handleClickRoot (value: String | __Type | [__Type] | __TypeKind | [__Field] | [__EnumValue] | [__InputValue] | null) {
  ...
  return (
    <>
      {stack.length === 1 && Object.entries(stack[stack.length - 1]).map(([key, value]) => {
        return (
          <div key={key}>
            <span>{`query: `}</span>
            <span onClick={() => handleClickRoot(value)} className={styles.click}>{JSON.stringify(value)}</span>
          </div>
          );
      })}

      {stack.length > 1 && stack[stack.length - 1].fields?.map((item, index) => {
        return (
          <div key={index}>
            <span>{`${item.name}: `}</span>
            <span className={styles.click}>{
              item.type.name === null ? String(item.type.kind) == __TypeKind[6] ? 'List' : `[${item.type.ofType?.name}]` : item.type.name
            }</span>
          </div>
          );
